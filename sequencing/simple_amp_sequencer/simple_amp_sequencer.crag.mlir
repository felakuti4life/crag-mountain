...
//
// Uses `metronome` and `beat_in_measure` funcs to compute the beat position
// within a measure, then selects one of four gain values (gain_0..gain_3)
// to scale the audio input.  This creates a simple rhythmic amplitude pattern
// that cycles every `beats_per_measure` beats.
//
// Block arguments:
...
module {
  ...
  ^bb0(%audio_in: !crag.audio<f32, 0, 1>):
    ...
    // Beat period in samples = sr * 60 / bpm
    ...
    // Beat 0 reference: sample 0
    %zero_i64   = arith.constant 0 : i64

    // Call metronome to detect beat events
    %fired, %at = crag.func_ref "metronome"(%zero_i64, %period_i64)
                      : (i64, i64) -> (i1, i32)
    ...
    // Get beat position within the measure (0-indexed)
